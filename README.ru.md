...
### Веб-интерфейс — разметка эпизодов

Первый экран внутреннего веб-интерфейса (раздел 6 ТЗ): ручная разметка «сработало / не сработало / рано судить» по срабатываниям сигналов. Это не продукт для пользователей — тот остаётся только в Telegram — а локальный инструмент для владельца: разметка вручную — единственный честный источник данных для будущей калибровки весов.

```powershell
.venv\Scripts\python.exe -m uvicorn web.app:app --host 127.0.0.1 --port 8000
```

Дальше открыть в браузере `http://127.0.0.1:8000/episodes`. Слушает только `127.0.0.1` — снаружи компьютера недоступен, авторизации нет и не планируется (границы записаны в `.claude/agents/web-dev.md`).

Один и тот же сигнал, срабатывающий каждый день неделями (например `revenue_price_gap` может держаться «сработавшим» неделями на одном протоколе), группируется в один эпизод, а не в отдельную строку на каждый день — группировку делает `storage/episodes.py`, переиспользуя ту же логику свежести, что и `scoring/ranker.py` для ежедневной сводки. На экране только эпизоды, которым хватило возраста для оценки (по умолчанию 7+ дней с начала, `config.yaml`'s `episode_review.min_age_days`) и ещё не размеченные; три кнопки пишут оценку прямо в новую таблицу `episode_outcomes` (`storage/db.py`) — `observations.md` этот экран не трогает, он остаётся отдельным журналом только для чтения. Эпизод, размеченный, пока ещё продолжался, всплывает на пересмотр, если он всё ещё идёт через `episode_review.reopen_after_days` (по умолчанию 14) дней — первая оценка могла не продержаться.

Формула `revenue_price_gap` изменилась 15 сентября 2026 (сумма недели → медиана, см. `BACKLOG.md`) — эпизод, пересекающий эту дату, всегда разбивается на два, а всё посчитанное по старой формуле помечено заметным бейджем и может быть отфильтровано, чтобы две формулы случайно не смешались в одной статистике.
